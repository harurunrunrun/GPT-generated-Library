...
- [Wavelet Matrix](#wavelet-matrix)
  ...
  - [WaveletMatrix<T>](#waveletmatrixt)
    ...
    - [総和クエリ](#総和クエリ)
    - [列挙・頻度分析](#列挙頻度分析)
    - [Bitwise 共通 API](#bitwise-共通-api)
    - [Bitwise 共通 API（変換後の値に対する検索・列挙）](#bitwise-共通-api変換後の値に対する検索列挙)
    - [XOR ラッパー](#xor-ラッハー)
    - [OR ラッパー](#or-ラッハー)
    - [AND ラッパー](#and-ラッハー)
  - [型エイリアス](#型エイリアス)
    - [`WaveletMatrix<T>`](#waveletmatrixt-1)
- [Weighted Wavelet Matrix](#weighted-wavelet-matrix)
  ...
  - [WeightedWaveletMatrix](#weightedwaveletmatrix)
    ...
    - [参照・出現回数・位置](#参照出現回数位置)
    - [順序統計・代表値](#順序統計代表値-1)
    - [値域条件での個数・重み和](#値域条件ての個数重み和)
    - [k 個の重み和](#k-個の重み和)
    - [前後探索](#前後探索-1)
    - [列挙](#列挙)
    - [XOR 変換後のクエリ](#xor-変換後のクエリ)
    - [OR 変換後のクエリ](#or-変換後のクエリ)
    - [AND 変換後のクエリ](#and-変換後のクエリ)
  - [RectangleSum](#rectanglesum)
    - [構築・メタ情報](#構築メタ情報-1)
    - [基本の矩形クエリ](#基本の矩形クエリ)
    - [x 範囲内での y 条件による個数・重み和](#x-範囲内ての-y-条件による個数重み和)
    - [x 範囲内での y の順序統計・探索](#x-範囲内ての-y-の順序統計探索)
    - [x 範囲内での y の列挙](#x-範囲内ての-y-の列挙)
    - [x 範囲内での XOR 変換 y クエリ](#x-範囲内ての-xor-変換-y-クエリ)
    - [x 範囲内での OR 変換 y クエリ](#x-範囲内ての-or-変換-y-クエリ)
    - [x 範囲内での AND 変換 y クエリ](#x-範囲内ての-and-変換-y-クエリ)
  ...
  - [型エイリアス](#型エイリアス-1)
    - [`WeightedWaveletMatrix`](#weightedwaveletmatrix-1)
    - [`RectangleSum`](#rectanglesum-1)
    - [`rect_monoid::*`](#rect_monoid)
    - [`StaticRangeMonoid`](#staticrangemonoid-1)
    - [`CommutativeMonoidWaveletMatrix`](#commutativemonoidwaveletmatrix-1)
    - [`RectangleMonoid`](#rectanglemonoid-1)
    - [便利エイリアス](#便利エイリアス)
